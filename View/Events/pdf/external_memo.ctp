<?php
	...
	} else if (substr( strtolower($event['Course']['code']), 0, 3 ) === "mrt") {
...
?>
		...
		<table style="border:none;text-align:left;width:100%;" cellpadding="0" cellspacing="0">
		    <tr>
		    	<td style="height:30px;" colspan="4">
		    		Our Ref. : RAD/ETM/(<?php echo $event['EventMemo'][0]['ref_number']; ?>)<?php echo $this->Time->format($event['EventMemo'][0]['modified'], '%Y'); ?>-<?php echo $event['Course']['code']; ?>
		    	</td>
		    </tr>
		    <tr>
			...
			<tr>
				<td style="height:25px;">
					<div style="text-align:justify;">
						We are pleased to inform that you have been nominated to attend the above-mentioned training and wish to confirm your participation as follows:
					</div>
				</td>
			</tr>
		</table>
		<table style="none;text-align:left;width:100%;padding-top:20px;" cellpadding="0" cellspacing="0">
			...
			<tr>
				...
				<td style="height:25px;width:400px;padding-left:30px;font-weight:bold;">
					<?php 
						// get training provider name
						$provider = $this->requestAction('/rail_competency/training_providers/object/'.$event['Course']['training_provider_id']);
						echo $provider['TrainingProvider']['name'];
					?>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td style="height:40px;">
					<div style="text-align:justify;line-height:25px;">
						The program will start promptly on time and your punctuality is highly appreciated.
					</div>
				</td>
			</tr>
		<?php if (!empty($event['EventMemo'][0]['memo'])) { ?>
			<tr>
				<td style="height:70px;">
					<div style="text-align:justify;">
						<?php echo $event['EventMemo'][0]['memo']; ?>
					</div>
				</td>
			</tr>
		<?php } ?>
...
<table style="none;text-align:left;width:100%;padding-top:20px;" cellpadding="0" cellspacing="0">
    ...
    <table>
   		<thead>
   			<tr>
	   		<th style="height:25px;width:25px;text-align:right;"> #</th>
   			<th style="height:25px;width:400px;text-align:left;"> Staff Name </th>
   			<th style="height:25px;width:200px;text-align:left;"> Department </th>
   		</tr>
   		</thead>
   		</tbody>
   		<?php $count = 1 ; ?>
